...
public class SnowGolemAbility implements IMobAbility {
    ...
    private static final int SNOWBALL_COOLDOWN_TICKS = 40;
    ...
    public void tick(ServerPlayer player, ItemStack totemStack) {
        ...
        if (now - lastSnowballTimes.getOrDefault(uuid, 0L) < SNOWBALL_COOLDOWN_TICKS) return;
    ...
    private void shootSnowball(Level level, ServerPlayer player, LivingEntity pTarget) {
        ...
        player.level().playSound(null, player.blockPosition(), SoundEvents.SNOW_GOLEM_SHOOT, SoundSource.PLAYERS, 1.0F, 0.4F / (player.getRandom().nextFloat() * 0.4F + 0.8F));
